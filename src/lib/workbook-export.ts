...
export type ExportSection = { heading: string; chapterTitle: string | null; html: string };
  ...
  const body = sections
    .map((section, i) => {
      ...
      return `
        ...
        ${chapterChanged && section.chapterTitle ? `<h1>${escapeHtml(section.chapterTitle)}</h1>` : ""}
